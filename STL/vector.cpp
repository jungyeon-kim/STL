...
/*
	push_back():			인자로서 필요한 임시객체를 생성 후 복사 or 이동이 일어난다.
	emplace_back():			인자로서 필요한 객체를 컨테이너에 바로 생성한다. (인자로 객체의 생성자 인자를 넘겨줬을 경우에만 한함 -> 템플릿 인자를 사용하기에 가능한 것)
	reserve():				()개가 들어갈 공간을 잡아준다. -> ()개까지 공간재할당(복사생성자호출)을 막아준다.
	erase():				삭제동작 후, 다음 iterator를 반환한다. size는 변하지만 capacity는 변함없다. (루프안에서 삭제동작시 주의가 필요하다.)
	erase(remove(), end()):	vector는 remove()가 내장되지 않아 특정값을 지울 수 없어 이와 같이 사용한다.
							...
							따라서, erase()를 이용해 remove() 반환값부터 end()전까지 데이터만 삭제 (capacity는 변함없음)
							※ 중복원소를 삭제하기위해 unique()를 사용할 때도 마찬가지
*/
...
void solution1()
{
	...
	for (auto i = v.cbegin(); i != v.cend(); ++i)	// 순회방법 2
		cout << *i << endl;	// 반복자는 포인터를 반환하기 문에 역참조로 값을 가져와야함.
}

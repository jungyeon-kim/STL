...
// ---------------------------------------------------------------------------------------------
// 순차컨테이너 "vector"
// 	- 값을 넣을 공간이 없으면 더 큰 공간(원래크기의 1.5배)을 재할당한다.
//	- 멤버함수 -> size, capacity, data (크기, 할당가능공간, 저장위치)
...
//	- erase(remove(), end()):	remove()가 해당 값들을 다음값으로 덮어씌운다. -> 의미없는 값들이 뒤에 생기고 그 영역의 시작을 가리키는 반복자 반환
//								-> erase()가 해당 값들의 remove() 반환값부터 end()까지 데이터만 삭제 (capacity는 변함없음)
// ---------------------------------------------------------------------------------------------
